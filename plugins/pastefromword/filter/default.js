(function(local){
	...
	var firstChild = function(evaluator){
		var child;
	...
	$E.htmlParser.fragment.implement({
	...
	$E.htmlParser.element.implement({
		firstChild: firstChild,
		
		onlyChild: onlyChild,

		// Adding a (set) of styles to the element's 'style' attributes.
		addStyle: function(name, value, isPrepend){
			...
			if (typeof value === 'string') addingStyleText += name + ':' + value + ';';
				...
				if (typeof name === 'object'){
					for (var style in name){
						if (name.hasOwnProperty(style)) addingStyleText += style + ':' + name[style] + ';';
					}
	...
	function postProcessList(list){
		var stylesFilter = $E.plugins.pastefromword.filters.stylesFilter, 
			styleTypeRegexp = /list-style-type:(.*?)(?:;|$)/,
			children = list.children,
			count = children.length,
			child, match, mergeStyle;

		var attributes = list.attributes;

		if (styleTypeRegexp.exec(attributes.style)) return;
		...
		if (mergeStyle){
			for (i = 0; i < count; i++){
				...
				attributes.style && (attributes.style = stylesFilter([['list-style-type']])(attributes.style) || '');
			}
	...
	var unorderedPatterns = {
		'disc': /[l\u00B7\u2002]/,
		'circle': /[\u006F\u00D8]/,
		'square': /[\u006E\u25C6]/
	};
		
	var listMarkerPatterns = {
		'ol': orderedPatterns,
	...
	function fromRoman(str){
		str = str.toUpperCase();
		var result = 0;
		for (var i = 0, l = romans.length; i < l; ++i){
	...
	function fromAlphabet(str){
		str = str.toUpperCase();
		var l = alpahbets.length, result = 1;
		for (var x = 1; str.length > 0; x *= l){
			result += alpahbets.indexOf(str.charAt(str.length - 1)) * x;
	...
	var plugin = ($E.plugins.pastefromword = {

		utils: {
			// Create a <kse:listbullet> which indicate an list item type.
			createListBulletMarker: function(bullet, bulletText){
				var marker = new $E.htmlParser.element('kse:listbullet');
				marker.attributes = {
					'kse:listsymbol': bullet[0]
				};
				marker.add(new $E.htmlParser.text(bulletText));
				return marker;
			},
			...
			isContainingOnlySpaces: function(element){
				var text = element.onlyChild();
				return text && (/^(:?\s|&nbsp;)+$/).test(text.value);
			},
			
			resolveList: function(element){
				// <kse:listbullet> indicate a list item.
				var attributes = element.attributes, listMarker;
				
				if ((listMarker = element.removeAnyChildWithName('kse:listbullet')) && listMarker.length && (listMarker = listMarker[0])){
					element.name = 'kse:li';
					
					if (attributes.style){
						attributes.style = plugin.filters.stylesFilter([
							...
							// Deal with component/short-hand form.
								...
								margin = $utils.convertToPx(values[3] || values[1] || values[0]);
								...
								attributes['kse:indent'] = listBaseIndent && (Math.ceil(margin / listBaseIndent) + 1) || 1;
							}],
							...
							[(/^mso-list$/), null, function(val){
								...
								if (indent == 1){
									listId !== previousListId && (attributes['kse:reset'] = 1);
									previousListId = listId;
								}
								attributes['kse:indent'] = indent;
							}]
						])(attributes.style, element) || '';
					}
					...
					if (!attributes['kse:indent']){
						previousListItemMargin = 0;
						attributes['kse:indent'] = 1;
					}
					
					// Inherit attributes from bullet.
					Object.merge(attributes, listMarker.attributes);
					return true;
				}
			...
			getStyleComponents: (function(){
				...
				var calculator = local.DOM.create('<div style="position:absolute;left:-9999px;top:-9999px;"></div>', document);
				document.body().append(calculator);
				
				return function(name, styleValue, fetchList){
					calculator.style(name, styleValue);
					var styles = {}, count = fetchList.length;
					for (var i = 0; i < count; i++) styles[fetchList[i]] = calculator.style(fetchList[i]);
					return styles;
				};
			})(),
		...
		filters: {
			...
			// <kse:li kse:listtype="ul" kse:indent="1">level1</kse:li>
			// <kse:li kse:listtype="ol" kse:indent="2">level2</kse:li>
					...
					if (child.name in $dtd.$listItem){
						...
						plugin.filters.stylesFilter([
							['tab-stops', null, function(val){
								...
								margin && (previousListItemMargin = $utils.convertToPx(margin[0]));
						...
						attributes['kse:indent'] = level;
						attributes['kse:listtype'] = element.name;
						attributes['kse:list-style-type'] = listStyleType;
					// Flatten sub list.
					} else if (child.name in $dtd.$list){
						...
						element.children = [];
						for (var j = 0, num = children.length; j < num; j++) element.add(children[j]);
					}
				}
			...
			assembleList: function(element){
				var children = element.children, child, listItem,
					// The current processing kse:li element. 
					listItemAttrs, listItemIndent,
				...
				for (var i = 0; i < children.length; i++){
					...
					if ('kse:li' == child.name){
						child.name = 'li';
						listItem = child;
						listItemAttrs = listItem.attributes;
						bullet = listItemAttrs['kse:listsymbol'];
						bullet = bullet && bullet.match(/^(?:[(]?)([^\s]+?)([.\u3001)]?)$/);
						listType = listStyleType = itemNumeric = null;
						
						if (listItemAttrs['kse:ignored']){
							children.splice(i--, 1);
							continue;
						}
						
						// This's from a new list root.
						listItemAttrs['kse:reset'] && (list = lastIndent = lastListItem = null);
						...
						listItemIndent = Number(listItemAttrs['kse:indent']);
						...
						if (!bullet){
							listType = listItemAttrs['kse:listtype'] || 'ol';
							listStyleType = listItemAttrs['kse:list-style-type'];
						} else {
						...
						if (!list){
							openedLists.push(list = new $E.htmlParser.element(listType));
							list.add(listItem);
							children[i] = list;
						} else {
			...
			stylesFilter: function(styles, whitelist){
				return function(styleText, element){
					...
					(styleText || '').replace(/&quot;/g, '"').replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function(match, name, value){
						...
						var namePattern, valuePattern, newValue, newName, style;
						for (var i = 0, l = styles.length; i < l; i++){
							if (style = styles[i]){
								namePattern = style[0];
								valuePattern = style[1];
								newValue = style[2];
								newName = style[3];
					...
					for (var i = 0, l = rules.length; i < l; i++) rules[i] = rules[i].join(':');
			...
			elementMigrateFilter: function(styleDefiniton, variables){
				return function(element){
					styleDefiniton = variables ? new $E.style(styleDefiniton, variables).styleDefinition : styleDefiniton;
					element.name = styleDefiniton.element;
					Object.merge(element.attributes, $utils.clone(styleDefiniton.attributes));
					element.addStyle($E.style.getStyleText(styleDefiniton));
				};
			},
			...
			styleMigrateFilter: function(styleDefinition, variableName){
				...
				return function(value, element){
					...
					var styleElement = new $E.htmlParser.element(null), variables = {};
			...
			/**
			 * A filter which remove kse-namespaced-attribute on
			 * all none-kse-namespaced elements.
			 * @param value
			 * @param element
			 */
			bogusAttrFilter: function(value, element){
				if (element.name.indexOf('kse:') == -1) return false;
			},
		...
		getRules: function(editor){
			var blockLike = Object.merge({}, $dtd.$block, $dtd.$listItem, $dtd.$tableContent), 
				...
				styleMigrateFilter = this.filters.styleMigrateFilter.bind(this.filters), 
			...
			return {
				...
				elements: {
					'^': function(element){
						...
						if (local.env.firefox && (applyStyleFilter = filters.applyStyleFilter)) applyStyleFilter(element);
					},
					...
					$: function(element){
						...
						if (tagName in blockLike && attributes.style){
							attributes.style = stylesFilter([[(/^(:?width|height)$/), null, convertToPx]])(attributes.style) || '';
						}
						...
						if (tagName.match(/h\d/)){
							...
							elementMigrateFilter(config.format[tagName])(element);
						...
						} else if (tagName.indexOf(':') != -1 && tagName.indexOf('kse') == -1){
							element.filterChildren();
							...
							if (styleDefText){
								...
								.replace(/(.+?)\{(.+?)\}/g, function(rule, selectors, styleBlock){
									...
									for (var i = 0; i < length; i++){
										...
										selectors[i].trim().replace(/^(\w+)(\.[\w-]+)?$/g, function(match, tagName, className){
											tagName = tagName || '*';
					...
					'p': function(element){
						...
						if (/MsoListParagraph/.exec(element.attributes['class'])){
							var bulletText = element.firstChild(function(node){
								return node.$text && !containsNothingButSpaces(node.parent);
							});
						...
						if (config.enterMode == $['ENTER_BR']){
							// We suffer from attribute/style lost in this situation.
							delete element.name;
							element.add(new $E.htmlParser.element('br'));
						} else elementMigrateFilter(config.format[(config.enterMode == $['ENTER_P'] ? 'p' : 'div')])(element);
					},
					...
					'div': function(element){
						...
						if (singleChild && singleChild.name === 'table'){
							...
							singleChild.attributes = Object.merge(singleChild.attributes, attributes);
							attributes.style && singleChild.addStyle(attributes.style);
							
							var clearFloatDiv = new $E.htmlParser.element('div');
					...
					'font': function(element){
						...
						var attributes = element.attributes, 
							styleText = attributes.style, 
							parent = element.parent;
						
						if ('font' == parent.name){
							// Merge nested <font> tags.
							Object.merge(parent.attributes, element.attributes);
						...
						// Convert the merged into a span with all attributes preserved.
							...
							if (attributes.color){
								attributes.color != '#000000' && (styleText += 'color:' + attributes.color + ';');
								delete attributes.color;
							}
							if (attributes.face){
								styleText += 'font-family:' + attributes.face + ';';
								delete attributes.face;
							}
							// TODO: Mapping size in ranges of xx-small,
							// x-small, small, medium, large, x-large, xx-large.
							if (attributes.size){
								styleText += 'font-size:' + (attributes.size > 3 ? 'large' : (attributes.size < 3 ? 'small' : 'medium')) + ';';
					...
					'span': function(element){
						...
						if (isListBulletIndicator(element)){
							...
							if (listType){
								...
								// those are to be removed later, now mark it with "kse:ignored".
								var ancestor = element.getAncestor('span');
								if (ancestor && (/ mso-hide:\s*all|display:\s*none /).test(ancestor.attributes.style)) marker.attributes['kse:ignored'] = 1;
								return marker;
							}
						}
						...
						var children = element.children, 
							attributes = element.attributes, 
							styleText = attributes && attributes.style, 
							firstChild = children && children[0];
						
						// Assume MS-Word mostly carry font related styles on <span>,
						// adapting them to editor's convention.
						if (styleText){
							attributes.style = stylesFilter([
								// Drop 'inline-height' style which make lines overlapping.
								['line-height'], 
								[(/^font-family$/), null, !removeFontStyles ? styleMigrateFilter(config.font.style, 'family') : null],
								[(/^font-size$/), null, !removeFontStyles ? styleMigrateFilter(config.fontsize.style, 'size') : null],
								[(/^color$/), null, !removeFontStyles ? styleMigrateFilter(config.color.forestyle, 'color') : null],
								[(/^background-color$/), null, !removeFontStyles ? styleMigrateFilter(config.color.backstyle, 'color') : null]
							])(styleText, element) || '';
						}
					...
					'b': elementMigrateFilter(config.basicstyles.bold),
					'i': elementMigrateFilter(config.basicstyles.italic),
					'u': elementMigrateFilter(config.basicstyles.underline),
					's': elementMigrateFilter(config.basicstyles.strike),
					'sup': elementMigrateFilter(config.basicstyles.superscript),
					'sub': elementMigrateFilter(config.basicstyles.subscript),
					// Editor doesn't support anchor with content currently (#3582),
					// drop such anchors with content preserved.
					'a': function(element){
						var attributes = element.attributes,
							href = (attributes && attributes.href) || '';
						if (attributes && !href && attributes.name) delete element.name;
						else if (local.env.webkit && href.match(/file:\/\/\/[\S]+#/i)) attributes.href = href.replace(/file:\/\/\/[^#]+/i, '');
					},
					'kse:listbullet': function(element){
						if (element.getAncestor(/h\d/) && !config.pasteFromWordNumberedHeadingToList) delete element.name;
					}
				},
					...
					'style': stylesFilter(removeStyles ?
						...
						[
							...
							[(/^margin$|^margin-(?!bottom|top)/), null, function(value, element, name){
								if (element.name in {p: 1, div: 1}){
									var indentStyleName = config.contentsDirection == 'ltr' ? 'margin-left' : 'margin-right';
						...
						[
							...
							[(/-color$/), null, function(value){
								...
								if (local.env.firefox) return value.replace(/-moz-use-text-color/g, 'transparent');
							}],
				...
				comment: local.env.ie ? falsyFilter : function(value, node){
					...
					if (local.env.firefox && imageInfo){
						var img = $E.htmlParser.fragment.fromHtml(imageInfo[0]).children[0], 
	...
	var pasteProcessor = new Class({
		...
		toHTML: function(data){
			var fragment = $E.htmlParser.fragment.fromHtml(data, false), 
	...
	$E.cleanWord = function(data, editor){
		// Firefox will be confused by those downlevel-revealed IE conditional
		// comments, fixing them first( convert it to upperlevel-revealed one ).
		// e.g. <![if !vml]>...<![endif]>
		if (local.env.firefox) data = data.replace(/(<!--\[if[^<]*?\])-->([\S\s]*?)<!--(\[endif\]-->)/gi, '$1$2$3');
		...
		dataFilter.addRules($E.plugins.pastefromword.getRules(editor));
		
		// Allow extending data filter rules.
		editor.fireEvent('beforeCleanWord', {
			filter: dataFilter
		});
		...
		// Remove 'kse' namespaced attribute used in filter rules as marker.
		data = data.replace(/kse:.*?".*?"/g, '');
 ...
 * @name Klass.Editor.config.pasteFromWordRemoveFontStyles
 * @since 3.1
 * @type Boolean
 * @default true
 * @example
 ...
 * @name Klass.Editor.config.pasteFromWordNumberedHeadingToList
 * @since 3.1
 * @type Boolean
 * @default false
 * @example
 ...
 * {@link Klass.Editor.config.pasteFromWordRemoveFontStyles} setting instead.
 * @name Klass.Editor.config.pasteFromWordRemoveStyles
 * @since 3.1
 * @type Boolean
 * @default true
 * @example
