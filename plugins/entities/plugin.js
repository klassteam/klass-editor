...
Klass.run(function(local){
	...
	constructor.plugins.implement('entities', function(editor){
		...
		var config = editor.config.entities;

		if (config.basic !== false) selectedEntities.push(htmlbase);
		
		if (config.entities){
			if (selectedEntities.length) selectedEntities.push(entities);
			
			if (config.latin) selectedEntities.push(latin);
			
			if (config.greek) selectedEntities.push(greek);
			
			if (config.additional) selectedEntities.push(config.additional);
		}
		...
		if (config.entities && config.processNumerical) entitiesRegex = '[^ -~]|' + entitiesRegex;
		...
		function getEntity(character){
			return config.processNumerical == 'force' || !entitiesTable[character] ? '&#' + character.charCodeAt(0) + ';' : entitiesTable[character];
		}
	...
	constructor.config.entities = {
		 ...
		 * @example
		 * config.entities.basic = false;
		 */
		basic: true,

		/**
		 * Whether to use HTML entities in the output.
		 * @name Klass.Editor.config.entities
		 * @type Boolean
		 * @default <code>true</code>
		 * @example
		 * config.entities.entire = false;
		 */
		entities: true,
		 ...
		 * @name Klass.Editor.config.entities.latin
		 * @type Boolean
		 * @default <code>true</code>
		 * @example
		 * config.entities.latin = false;
		 */
		latin: true,
		 ...
		 * @name Klass.Editor.config.entities.greek
		 * @type Boolean
		 * @default <code>true</code>
		 * @example
		 * config.entities.greek = false;
		 */
		greek: true,
		 ...
		 * @name Klass.Editor.config.entities_processNumerical
		 * @type Boolean|String
		 * @default <code>false</code>
		 * @example
		 * config.entities.processNumerical = true;
		 * config.entities.processNumerical = 'force';		//Converts from "&nbsp;" into "&#160;";
		 */
		 ...
		 * @name Klass.Editor.config.entities.additional
		 * @default <code>'#39'</code>  (The single quote (') character.)
		 * @type String
		 * @example
		 * config.entities.additional = '#1049';		// Adds Cyrillic capital letter Short I (&#1049;).
		 */
		additional: '#39'
